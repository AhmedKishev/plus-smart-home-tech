...
# Модуль infra
Данный модуль содержит всю инфраструктуру приложения. Содержит 3 подмодуля, а именно:

## • Config-server
Этот сервис представляет собой Spring-Config приложение, то есть содержит конфиги для каждого из сервисов.

## • Discovery-server
Данный сервис является реестром сервисов и, соответственно, использует Spring Cloud Eureka. 
