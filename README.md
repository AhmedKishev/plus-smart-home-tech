...
## • Shopping store
Данный сервис представляет собой витрину товаров, из которых пользователи выбирают товары для заказа. Он управляется администратором. 

## • Shopping cart 
Сервис `shopping cart` отвечает за работу с корзиной пользователей. Пользователи будут добавлять товары в корзину, чтобы сделать заказ. При этом бронирование происходит посредством вызова склада через feign-клиент. 

## • Order
Сервис `order` работает с заказами пользователей, оформляет их и они собираются из склада. После этого они передаются в доставку в сервис `delivery `.

## • Warehouse
Данный сервис представляет собой склад. Им также управляет администратор. 

## • Payment
В этом сервисе сосредоточена логика, связанная с оплатой заказов. Она непосредственна связана с заказами. 
